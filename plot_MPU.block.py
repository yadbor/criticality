...
pens = [pg.mkPen(width=5, color='rgw'[i]) for i in range(nComponents)]
...
curve1 = p1.plot(ax, pen=pens[x])
curve2 = p1.plot(ay, pen=pens[y])
curve3 = p1.plot(az, pen=pens[z])
curve4 = p4.plot(gx, pen=pens[x])
curve5 = p4.plot(gy, pen=pens[y])
curve6 = p4.plot(gz, pen=pens[z])
# curve7 = p7.plot(yaw)
# curve8 = p8.plot(roll)
    ...
    def test_method(self):
        try:
            while(not self.stopper.wait(1)):
                ...
                getData()
            print("self stopper loop exited")
        finally:
